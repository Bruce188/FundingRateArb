...
public class ExchangeAnalyticsPageTests : IClassFixture<ExchangeAnalyticsPageTests.AnalyticsTestFactory>, IDisposable
{
    private readonly HttpClient _client;

    public ExchangeAnalyticsPageTests(AnalyticsTestFactory factory)
    {
        SeedData(factory);
    ...
    [Fact]
    public async Task ExchangeAnalyticsPage_ReturnsSuccess()
    {
        var response = await _client.GetAsync("/Admin/ExchangeAnalytics");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var html = await response.Content.ReadAsStringAsync();
        html.Should().Contain("Exchange Analytics");
        html.Should().Contain("Exchange Overview");
    }
    ...
    public class AnalyticsTestFactory : WebApplicationFactory<Program>
    {
        private static readonly string DbName = $"AnalyticsTest_{Guid.NewGuid()}";
